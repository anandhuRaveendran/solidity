...
std::vector<size_t> const& SSACFGLoopNestingForest::loopParents() const { return m_loopParents; }
std::set<size_t> const& SSACFGLoopNestingForest::loopNodes() const { return m_loopNodes; }
std::set<size_t> const& SSACFGLoopNestingForest::loopRootNodes() { return m_loopRootNodes; }
...
void SSACFGLoopNestingForest::findLoop(size_t const _potentialHeader)
{
	if (m_sort.backEdgeTargets().count(_potentialHeader) > 0)
	{
		...
		for (auto const pred: m_sort.predecessors()[_potentialHeader])
		{
